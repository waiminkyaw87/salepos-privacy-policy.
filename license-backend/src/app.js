...
app.post("/v1/license/mmqr/create-order", requireAuth, asyncHandler(async (req, res) => {
  ...
  // Extract from response or response.data (some SDKs wrap it)
  const data = paymentResponse?.data || paymentResponse;
  const createdOrderId = normalize(data?.orderId) || orderId;
  const status = normalize(data?.status || "PENDING").toUpperCase();
...
app.post("/v1/license/mmqr/callback", asyncHandler(async (req, res) => {
  ...
  console.log(`[MMQR] Callback received. Signature present: ${!!signature}, Nonce present: ${!!nonce}`);
  ...
  if (!signature || !nonce) {
    console.warn("[MMQR] Callback missing signature or nonce headers.");
  }

  const verified = await verifyMmqrCallback(payloadString, nonce, signature);
  if (!verified) {
    console.warn("[MMQR] Callback verification failed. Payload snippet:", payloadString.substring(0, 100));
    return res.status(401).json({ error: "MMQR callback signature verification failed." });
  }

  const payload = JSON.parse(payloadString || "{}");
  const orderId = extractOrderId(payload);
  console.log(`[MMQR] Callback verified for orderId: ${orderId}, status: ${payload.status}, condition: ${payload.condition}`);
