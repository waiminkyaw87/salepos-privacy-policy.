...
function isMmqrSuccess(orderStatus, condition) {
  ...
  const normalizedCondition = normalize(condition).toUpperCase();
  if (status !== "SUCCESS") {
    return false;
  }
  return !normalizedCondition || normalizedCondition === "PRISTINE";
}
...
function generateMmqrOrderId(uid) {
  ...
  return `SP-MMQR-${Date.now()}-${normalize(uid).slice(-6)}-${entropy}`;
}
...
app.get("/v1/license/mmqr/order/:orderId", requireAuth, asyncHandler(async (req, res) => {
  ...
  if (!order) {
    ...
    return res.status(404).json({
      error: "QR code is not available yet. Please wait a few seconds and try again. If the problem persists, ensure your payment order was created successfully."
    });
  }
