...
- Optional MMQR payment flow with server-side callback verification
...
- `POST /v1/license/mmqr/create-order`
- `GET /v1/license/mmqr/order/:orderId`
- `POST /v1/license/mmqr/callback`
...
- Optional plan overrides:
  ...
  - `BUSINESS_PRODUCT_IDS=salespos_business,salepos_growth`
  - `PRO_PRODUCT_IDS=salepos_pro`
### MMQR setup (second billing option)

The backend supports MMQR via `mmpay-node-sdk` and a secure callback endpoint.

Required env vars:

- `MMQR_ENABLED=true`
- `MMQR_APP_ID=<from MyanMyanPay>`
- `MMQR_PUBLISHABLE_KEY=<pk_...>`
- `MMQR_SECRET_KEY=<sk_...>`
- `MMQR_CALLBACK_URL=https://<your-backend>/v1/license/mmqr/callback`

Recommended env vars:

- `MMQR_SANDBOX_ENABLED=true` for testing
- `STARTER_AMOUNT_MMK`, `BUSINESS_AMOUNT_MMK`, `PRO_AMOUNT_MMK`
- `MMQR_ORDERS_COLLECTION=mmqrOrders` (Firestore mode)

Security notes:

- Callback signature + nonce are verified using MMQR SDK (`verifyCb`)
- Entitlement is updated only after callback status is `SUCCESS`
- Callback endpoint does not require Firebase auth by design (provider webhook)
